...
### 2026-07-19 — Phase 4 kickoff: the clean break *(independence)*

Phase 3 is **complete** — the `src/` rewrite passes the whole corpus as its own reference
implementation — so Phase 4's headline act landed: the legacy tree is **deleted**. Gone are `lib/`
(the ~20k-line Babel-transpiled ExcelJS), the mocha/jasmine `spec/`, ~55 legacy `test/*.js` scratch
scripts and their fixtures, the Babel/Grunt/Browserify/ESLint-6/prettier-eslint toolchain, and the
legacy corpus oracle (`current.mjs` / `workbook-io.mjs` — the only code that reached into `lib/`).
First the corpus was **re-founded on the rewrite**: the runner defaults to `--adapter rewrite`
(wired as the sole `npm run corpus`; `corpus:rewrite` retired), and the 233 `baseline: 'fail'`
behaviours the rewrite already resolved were flipped to `'pass'` — the marker survives only as a
deliberate pending flag for future not-yet-built capabilities. Then `package.json` was rebuilt on the
rewrite: runtime deps are **`fflate` alone** (down from ten legacy runtime deps), entry points resolve
to `src/index.ts`, scripts are the real gates (`typecheck` / `test:src` / `corpus`), the strict
tsconfig is promoted to the default `tsconfig.json`, and CI drops the grunt/asset-size jobs. The
package is marked `private` and versioned `0.0.0-dev` — it is source-only (`exports` → `./src/index.ts`,
`engines.node >=24`) until the publishable build pipeline lands. `typecheck` clean; `test:src`
**647 pass**; `corpus` **671 green, 0 regressions**. Net: **543 files, −59,422 lines.**

**Remaining in Phase 4 (order in `STRATEGY.md`):** (1) publishable build pipeline — emit ESM `.js` +
`.d.ts`, widen `engines`, drop `private`, bundle-size budget in CI; (2) toolchain standup — Biome
lint/format, keep-`node --test`-or-adopt-Vitest decision (ADR) + type-level tests; (3) docs generated
