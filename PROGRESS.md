...
- **Latest corpus vs rewrite: 198 green / 5 known-open / 88 legacy known-opens resolved / 0 regressions /
  370 skipped; test:src 258/258.** The **page-setup slice** teaches the sheet its print scaling: a new
  `PageSetup` on `Worksheet.pageSetup` (`fitToPage`/`fitToWidth`/`fitToHeight`/`scale`/`orientation`/`pageOrder`,
  mutable in place) lets an author fit columns/rows onto a fixed page count or a fixed zoom and pick paper
  orientation and page order. The fit-to-page flag rides `<pageSetUpPr>` inside `<sheetPr>` (after `<outlinePr>`
  ...
  `fitToPage` unset, model clone). Not yet modelled from `<pageSetup>`: `paperSize` and the printer-settings
  `r:id` (dropped on write — the fidelity case does not assert them; a faithful passthrough is a future slice).
