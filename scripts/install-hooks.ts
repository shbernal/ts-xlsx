...
//
// `prepare` runs this on every `pnpm install`. Since pnpm 11 verifies dependencies before
...
// `core.hooksPath` points somewhere it does not own, and exits 1. That is correct: writing its
 ...
 * applies to path-typed values have to be reapplied here: `~` expansion, and resolving a relative
 * path against the top of the working tree, where hooks are run from.
 */
