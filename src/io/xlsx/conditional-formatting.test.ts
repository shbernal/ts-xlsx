...
test('a dataBar rule reads back on the same range with its type, colour, and both anchors', () => {
  ...
  assert.deepEqual(rule?.cfvo?.map(v => v.value), [0, 1], 'both numeric anchors survive in order');
});

test('a minimal dataBar (no cfvo, no colour) gains Excel default min/max anchors and a bar colour', () => {
  const workbook = new Workbook();
  workbook.addWorksheet('S').addConditionalFormatting({ref: 'A1:A3', rules: [{type: 'dataBar', priority: 1}]});
  const block = sheetXml(writeXlsx(workbook)).match(/<dataBar[\s\S]*?<\/dataBar>/)?.[0] ?? '';

  assert.equal([...block.matchAll(/<cfvo\b/g)].length, 2, 'a default data bar carries a min and a max cfvo');
...
test('a rule over a multi-area ref emits one block whose sqref lists every area', () => {
  ...
  workbook.addWorksheet('S').addConditionalFormatting({
    ...
    rules: [{type: 'colorScale', cfvo: [{type: 'min'}, {type: 'max'}], colors: [{argb: 'FFFF0000'}, {argb: 'FF00FF00'}]}],
  });
  const xml = sheetXml(writeXlsx(workbook));

  assert.equal([...xml.matchAll(/<conditionalFormatting\b/g)].length, 1, 'one block, not one per area');
...
test('a stopIfTrue rule serialises the flag, references a dxf for its style, and both survive a read', () => {
  ...
  assert.match(xml, /<cfRule type="cellIs" dxfId="0" priority="1" stopIfTrue="1" operator="greaterThan">/);
  assert.match(stylesXml(pkg), /<dxfs count="1"><dxf><fill><patternFill patternType="solid"><bgColor rgb="FFFF0000"\/>/);
...
test('an expression rule with no formula serialises without throwing', () => {
  const workbook = new Workbook();
  workbook.addWorksheet('S').addConditionalFormatting({ref: 'A1:A10', rules: [{type: 'expression', style: {}}]});
  const xml = sheetXml(writeXlsx(workbook));
  assert.match(xml, /<cfRule type="expression"[^>]*\/>/, 'a formula-less rule is a self-closing cfRule');
});
...
test('a self-closing rule read from a file (a duplicateValues) round-trips its type, dxfId, and priority', () => {
  ...
  assert.match(xml, /<cfRule type="duplicateValues" dxfId="0" priority="1"\/>/, 'the rule survives, not dropped');
...
test('a foreign differential style with a custom number format round-trips verbatim, never "[object Object]"', () => {
  ...
  assert.doesNotMatch(out, /\[object Object\]/, 'the dxf numFmt is a real format code, not a coerced object');
