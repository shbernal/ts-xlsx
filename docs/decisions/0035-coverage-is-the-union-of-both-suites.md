# 0035 — Coverage is the union of both suites, measured with node's own implementation
...
**not** yet a CI gate with a threshold (deliberate — a threshold is a follow-up once we
...
suites — 16.8 s → 32.5 s for the units, 20.7 s → 39.8 s for the corpus — so folding it in
...
cheaper risk. Node's type-stripping is what makes this work at all — offsets in the
V8 data land on the original `.ts` bytes, since stripping preserves positions.

## Alternatives rejected

- **Reimplement the line/branch mapping.** Independent of node internals, but produces
...
- **Point `node --test --experimental-test-coverage` at a pre-populated directory.**
  ...
  *"so that no preexisting coverage files interfere with the results"*. Not a bug to work
  around — it is the runner protecting its own report.
...
- **Positive:** the reported number is the library's number. The floor ADR 0029 deferred
  ...
  No new dependency — the implementation is the one already in the runtime.
- **Negative / accepted:** a private node API is on the path, and will break on some
  future node — loudly, by design. A full run costs ~74 s, so it is on demand, not in
