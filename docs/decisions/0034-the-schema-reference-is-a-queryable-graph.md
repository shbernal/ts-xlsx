...
> **An ECMA-376 MCP.** None exists; the standard is a static PDF plus XSD, correctly handled
> by vendoring.
...
`ooxml-lookup` is pre-1.0 and its answers may change shape without a deprecation period, and
