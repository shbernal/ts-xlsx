...
> NOT crash it, so the exact malformed shape that triggers the throw is version- and shape-specific and
> not reliably reproducible. The durable requirement, that the writer must never crash on a malformed
...
- Serializing a workbook **never crashes with an unguarded undefined-property access** when a cell
  carries a malformed or incomplete rich-text-shaped value. The writer either:
  1. treats a rich-text value with a missing or empty runs array as an **empty, blank cell**, or
  2. **fails fast with a precise, actionable error** that names the offending **sheet and cell
     address** and states that the rich-text value is missing its runs array.
- Silent deep-stack `TypeError`s that do not identify the cell are unacceptable, because the whole
  point is that a caller building values programmatically can find the one bad cell.
...
- Default policy: tolerate-as-blank, which is robust but hides a caller bug, or fail-fast-with-location,
  which surfaces the bug? A strict or validate mode could choose fail-fast while the default tolerates.
...
- Should the type surface make a malformed rich-text value unrepresentable, with a precise `RichText`
