...
A user parses a very large workbook, tens of megabytes and too big to buffer, with the streaming
...
> design question, about when merges are known relative to the rows they cover, not a malformed-output
...
- **The streaming reader surfaces every merged range** a worksheet declares, as part of the same
  worksheet's stream, either as an event or property on the worksheet reader or attached to the emitted
...
- **Feature parity is the invariant**: a merged-cell range visible to the buffered reader is visible
  to the streaming reader too, and the two I/O strategies do not disagree about the document
...
- **Ordering is defined**: because `mergeCells` follows the sheet data in the XML, the reader states
  ...
  streaming row-by-row knows when it can trust the merge set, buffering the ranges and applying at
  sheet end if they arrive last.
