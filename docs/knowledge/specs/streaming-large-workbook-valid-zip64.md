...
bogus signature value varying by data. A varying signature points at the ZIP container, meaning wrong
central-directory or end-of-central-directory offsets, or missing ZIP64 records once the archive or
an entry crosses the classic 4 GB / 0xFFFFFFFF offset boundary, not at the spreadsheet XML. It
...
> size or offset boundary, and the reporter used ten million rows, which is far too large to write in
> CI without OOM or timeout. A hostile-or-huge repro belongs in a spec note; a corpus case would stall
> the runner. Moderate streamed workbooks already produce valid, re-readable ZIPs, locked by
...
- **ZIP64 is emitted when needed.** Once the archive, an entry's compressed or uncompressed size, an
  entry offset, or the entry count crosses the classic 32-bit limits, the writer emits the ZIP64
  end-of-central-directory record and locator and the per-entry ZIP64 extra fields, so offsets and
  sizes beyond 0xFFFFFFFF are represented correctly rather than truncated into a bogus offset.
...
- **Over-limit row counts fail loudly at write time.** When the requested rows exceed the format
  maximum of 1,048,576 per worksheet, the writer raises a clear, typed error when the limit is crossed
...
- Whether the underlying zip layer, today's dependency or the planned lean-zip rewrite, emits ZIP64
  correctly, or whether ZIP64 support is the actual gap. This ties directly to
  `lean-zip-container-strategy`.
- What a meaningful large-ish smoke test looks like: big enough to exercise the beyond-4 GB and
  beyond-0xFFFFFFFF offset path, or at least the entry-count and streaming-CRC paths, without OOM or
  timeout on CI, possibly gated behind an opt-in "slow" tag rather than the default corpus run.
- Whether to expose a streaming-time progress or size signal so a caller can detect approaching limits,
