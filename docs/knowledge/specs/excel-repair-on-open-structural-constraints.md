...
3. **Dropped-but-referenced part dangles a reference. (Fixed.)** A no-op load→save of a workbook with an external-workbook link dropped the `xl/externalLinks/externalLink1.xml` part (and its `<externalReferences>` registration) while *keeping* the `[1]…`-using formulas and defined names — a dangling external reference Excel repairs on open. This was the single structural break behind the repair prompt on a rich real-world workbook (a referential-integrity sweep of the re-emitted package found no other). Fixed by preserving external links through the workbook preserved-reference net — the general lesson: the writer regenerates rels/content-types/`workbook.xml` from the model, so any part it neither models nor preserves is dropped, and if a *retained* element still points at it, the result is a dangling reference. See `external-workbook-reference-formulas`; locked by the `external-workbook-link-survives-roundtrip` corpus case. (calcChain, customXml, and sharedStrings also drop on this path but are referenced by nothing that survives, so Excel tolerates them — a dropped part only forces a repair when something still points at it.)
