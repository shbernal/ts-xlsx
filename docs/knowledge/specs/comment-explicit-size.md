...
> Spec note, not a corpus case: this is an authoring addition (accept explicit dimensions on
> a note) and a decision about units and precedence against auto-fit, a design question rather than a
...
- Authoring unit: pixels, converted to VML `pt` or EMU internally, against exposing the raw format
  units. Pixels match user intuition and the screenshots in the requests.
- Anchor interaction: is the size absolute, or expressed as a cell-span (from and to anchor) the way
  floating shapes are? Absolute width and height is simpler and matches the request; a cell-span
  anchor is a possible richer alternative.
