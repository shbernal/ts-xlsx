...
`s=` indices, and drawing, comment and table parts by relationship id, all of which differ between
the two packages.
...
- **Emit one worksheet's part.** Serialize an individual worksheet to its OOXML worksheet-part
...
- **Sheet swap.** A higher-level operation that replaces a single sheet part inside an existing
  ...
  version of the manual unzip, copy and rezip workaround.
- Both are bounded and streaming-friendly so a large sheet can be emitted without buffering the whole
  workbook, which ties to the streaming-write family.

## Open questions

- Does the emit-one-sheet API take a live worksheet from a loaded workbook, so it can resolve shared
  strings and styles, or a standalone sheet plus an explicit shared-string and style context?
- Sheet swap conflict policy: merge the incoming sheet's shared strings and styles into the target's
...
- How are relationship-backed sheet features (drawings, comments, tables) on the incoming sheet
  carried across: remapped rel ids plus copied parts, or rejected if present?
- Is this preferable to the more general `roundtrip-preserves-unmodeled-package-parts` approach,
  preserving everything the model doesn't touch, or complementary?
