...
The worksheet API exposes a single-column accessor and a range-of-rows accessor, getting one row or
a contiguous block of rows, but no symmetric range-of-columns accessor. Callers who want to
...
setting width, hidden and numFmt and for per-cell access.
...
- ~~**Signature:** `(start, count)` against `(firstCol, lastCol)`~~ **Settled by precedent: first and
  ...
  of a pair of numbers across all three axes, with no start-and-count overload beside it.
- **Column identity:** accept 1-based numeric indices, or letter references, consistently with
  the single-column accessor.
- **Out-of-range or non-positive count:** define whether an empty array, `undefined`, or an error
  is returned, and state it explicitly rather than inheriting the row accessor's behavior implicitly.
...
Related note: `worksheet-columns-mutable-array-ergonomics`, since both concern making the column axis
