...
- Expose a first-class way to set the document's default font once, rather than forcing
  per-cell, per-row or per-column styling. Two natural levels:
  - **Workbook-level default font**, the base font applied to all sheets unless overridden. This is
    ...
    cells**.
  - **Optional worksheet-level override**, for example a default or style option when adding a
    worksheet, shaped like `addWorksheet(name, { style: { font: { size, name } } })`.
...
- Post-processing every cell after writing to stamp `font.size` and `font.name` on any missing them,
  which is O(cells) and misses truly-empty cells unless iterating with `includeEmpty`.
- Monkey-patching the styles serializer to inject a font entry, which is fragile and reaches into
  internals.
...
- Precedence and merge: workbook default against worksheet default against column/row default against
  explicit cell font. Confirm the inheritance chain, and whether partial fonts merge (set only size,
  keep default name) or replace wholesale.
- Does setting a workbook default rewrite the theme's minor and major font, or only the styles font
  table? This affects whether Excel labels it a theme font or a direct font.
- Interaction with the shared-strings and inline default, and with rich-text runs carrying their own
  fonts.
