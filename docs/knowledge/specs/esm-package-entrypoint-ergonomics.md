...
constructs the top-level workbook type. Depending on bundler and module system, whether a
webpack-backed React and TypeScript app, a vite or remix build, a nuxt/nitro server, plain Node ESM,
...
the named class off a default import, swapping namespace against default import syntax, or toggling
...
(NodeNext or bundler resolution), and the common bundlers (webpack, vite, rollup, esbuild, remix,
...
default-against-namespace swaps) are symptoms of that packaging, not fixes the library should
require.

## How the fork precludes this

Ship real ESM as the source of truth with named exports. If CJS is supported at all, provide a
correct dual export map (`exports` with `import`, `require` and `types` conditions); otherwise drop
CJS entirely per the fork's ESM-only stance and eliminate the interop join. Ensure the emitted
...
across resolvers and bundlers, so a regression cannot ship. The plain named import must be the easy
...
- Does the fork support CJS consumers at all, or is it strictly ESM, which makes the dual export
  map unnecessary and removes the interop join entirely?
