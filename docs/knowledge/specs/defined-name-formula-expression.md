...
expressions, not only plain cell or range references. Excel stores every defined name as a formula
string, and a range reference like `Sheet1!$A$2:$A$6` is merely the simplest such expression. The
standard technique for cascading or dynamic dropdowns is a defined name whose definition is a dynamic
range built with `INDEX`/`MATCH` or `OFFSET`:
...
> Spec note, not a corpus case: this is a feature gap, since the defined-names API accepts only ranges,
...
- Public API shape: a single `formula` or `definition` string field on a defined name, with a plain
  range accepted as one formula shape, against keeping a range-oriented convenience. Prefer
  formula-first.
...
- How absolute and relative references and cross-sheet qualification are normalized on round-trip, if
  at all. Verbatim preservation is the safest baseline.
