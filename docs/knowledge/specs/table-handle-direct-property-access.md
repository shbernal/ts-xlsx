...
> behavior. The durable value is the desired handle contract and the naming and mutation decisions.

## Desired behavior

- The create and retrieve accessors return **one** table handle object, not a wrapper bundling the
...
- The handle's documented properties (column definitions, anchoring reference range, rows, applied
  style or theme) are directly accessible and consistent with the declared types. A read-after-write
...
- `add`, `get`, and `remove` share **one table-identity concept**, by name, so their
  return and argument types line up as a coherent trio.

## Open questions

- The reference-range property name: the reference implementation used both `ref` in the interface
  and `tableRef` at runtime, and these collapse to one honest name.
- Live view or snapshot: should the handle be a live view, where mutations flow into the workbook
  model, or an immutable snapshot with explicit mutation methods? Given the fork's
  immutable-by-default stance, prefer explicit typed mutation entry points over free property
  assignment, while the **read** surface still exposes `columns`, `ref`, `rows` and `style` directly.
