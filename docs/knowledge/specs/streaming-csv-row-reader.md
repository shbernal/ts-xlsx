...
bulk-only, since the whole file is parsed into a worksheet, so a large CSV cannot be consumed
incrementally.
...
> the desired API and its semantics, and the recorded gap, that bulk CSV read exists while
> bounded-memory streaming CSV read does not, even though streaming already exists for the package
> format.

## Desired behavior

- **Rows as an async iterable.** A streaming CSV reader yields parsed rows so callers can
  `for await (const row of reader) { … }` over a large CSV file or stream without loading the entire
  file. Each yielded row surfaces its cell values in order, and ideally a row number.
...
- **Option parity with the bulk reader.** Parsing options that apply to non-streaming CSV reads
  (delimiter, quote handling, encoding, date parsing and format hints, header handling) apply equally
  to the streaming path, sharing one option vocabulary.

- **Backpressure and early termination.** Iteration respects consumer pace and allows `break` to stop
  reading early, releasing the input without draining the whole file, so the bounded-memory promise
  holds in practice, not just in shape.

- **Modern input sources.** Source at least a file path, for convenience, and a Node `Readable`; a Web
...
- API shape: mirror the package reader's two-level iteration for symmetry, or expose a single-level row
  ...
  surface.
- Row shape: yield raw value arrays, or richer row objects with typing and coercion for numbers and
  dates consistent with the non-streaming reader? Consistency with the bulk reader argues for the
  latter.
- Header rows: an option to treat the first row as a header and expose subsequent rows keyed by column
  name.
- How this unifies with the broader streaming and buffered IO surface so CSV and package streaming
  share one consumption idiom (see `unified-streaming-and-buffered-io`).
