...
applications reject. In the reported cases the symptom traced to three non-library causes. First,
the write promise was never awaited, so the process moved on, or the file was served or downloaded,
before bytes were flushed. Second, the target directory was read-only in the deployment environment,
so the write silently produced no content. Third, a batch job fired hundreds to thousands of
concurrent write calls in a tight loop with no backpressure, exhausting file handles and memory and
leaving many outputs empty. Users also saw the on-disk file momentarily at zero bytes and only later
at its true size, because the write is not atomic, so a consumer reading the path mid-write sees a
partial file.
...
the buffered path was where users hit zero-byte and hang symptoms under high volume, and the
...
- Should the default file-write always be atomic (temp plus rename), given the extra fsync and
  rename cost?
...
- Exact error contract when the target path or directory is not writable.
