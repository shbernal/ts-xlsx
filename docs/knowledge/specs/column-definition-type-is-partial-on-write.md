...
- The read-side column returned by the library may be a richer object, fully populated with derived
  properties and helper methods, but callers are never required to construct that shape.
...
model: **input is partial and optional; the full, required shape belongs only to read-back.** A cleaner
...
- Which fields are legitimately settable per-column at definition time? Confirm the set (`header`,
  `key`, `width`, `outlineLevel`, `hidden`, `style`), where `header` may be a single string or an
  array of strings for multi-row headers.
- Mirror the same partial-input principle for rows and cells so the whole authoring API is
