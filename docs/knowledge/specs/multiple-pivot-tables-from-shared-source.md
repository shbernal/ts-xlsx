...
and values layout. The pivot feature must not cap a workbook at a single pivot table. Adding a second
or Nth pivot table, over the same source range as an existing one or a different source, must
...
2. **A distinct identity (UUID) per pivot table.** A reused pivot-table GUID collapses multiple
   tables into one, so generate a fresh unique id per table.
3. **Complete cache field and sharedItems set.** A cache built only from the fields one table happens
   to use is insufficient when another table over the same source uses other fields, so the cache
   should cover all source fields and let any field appear in any layout backed by it.
4. **Correct, unique per-table worksheet relationships.** Relationship targets must not be hardcoded
   ...
   unique rel-ids, verifiable via worksheet-rels or package-part inspection.
5. **Non-mutating cache-field extraction.** Building the cache fields must not mutate the source
   ...
   second table was created, and repeated pivot creation over the same source must stay correct.

## Open questions

- **Cache sharing model:** when multiple tables target the identical source range, share one
  pivotCache, which is Excel's usual behavior and gives smaller files with coupled refresh, or give
  each a private cache? A cleaner design may dedupe caches by source range rather than always
  one-per-table.
- **Public API shape:** design it fresh, as a workbook-level pivot builder or `addPivotTable`
  with `sourceSheet`, `rows`, `columns` and `values`, rather than inheriting the experimental legacy
  shape.
- **Validation over prohibition:** instead of forbidding N greater than 1, validate that source
  ranges, field names, and layouts are coherent and surface actionable errors.
