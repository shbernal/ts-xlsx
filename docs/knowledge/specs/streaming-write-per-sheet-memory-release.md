...
workaround, writing N separate single-sheet workbooks and merging, confirms the constraint
...
per-row commit logic itself is correct, and the retention lives in the archive layer's
serial consumption of entry streams.

## Open questions for the rebuild

- Does the planned zip layer support concurrent or interleaved entry writes? If yes, true
  per-sheet streaming is first-class. If it must serialize entries, the streaming API
  should either document that interleaved multi-sheet writing is bounded only by
  explicitly ordering sheet commits, or spill overflow committed rows for
...
- This is the write-side companion to the read-side memory contract in
  [[bounded-memory-large-workbook-read]], and the two should share a documented memory model.
- Not a corpus case: memory release is a resource property, not a JSON-serializable
  observable output, so it belongs in the streaming design spec.
