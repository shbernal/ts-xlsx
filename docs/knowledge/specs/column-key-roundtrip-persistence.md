...
A user defines columns with named keys (`id`, `name`, `dob`) plus header text and width,
...
The `<col>` elements carry width, hidden and style; header text is just cell content in the first
...
1. **Reassign on read.** The caller re-sets `columns` with the same key definitions after loading.
   Works but is unergonomic and error-prone, and nothing is actually persisted.
...
3. **Store keys in a package-level custom part or document metadata.** Survives our own
...
- If we persist keys, where (a sheet-XML attribute or a custom package part) and how do we avoid
...
Whatever we choose, the contract must be explicit and typed in the public API so callers are
