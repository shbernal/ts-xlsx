...
A developer assigns a JavaScript `Date` to a cell and expects the wall-clock date and time they
constructed to appear verbatim in the spreadsheet. Excel and OOXML store dates as timezone-naive
...
the instant's UTC components, a caller in a non-UTC zone who wrote a local-midnight date such as
...
- Offer an explicit conversion policy at write time:
  - a default **UTC projection** (serial equals the instant's UTC components), interoperable with
    foreign generators that also serialize naive dates, and matching historical behavior;
  - an opt-in **local / host-timezone projection** (serial equals local wall-clock components),
    covering the most common surprise;
  - ideally an explicit **named-zone or fixed-offset projection** so the same input plus declared zone
...
- Pre-shift the instant so its UTC components equal the desired wall-clock:
  `new Date(d.getTime() - d.getTimezoneOffset()*60000)`. This makes local wall-clock survive, at the
...
- Reinterpret local time as UTC without moving the timestamp (Luxon
  `setZone('utc', {keepLocalTime:true})`, moment or dayjs `utcOffset`).
...
- Accept only `Date`, or also epoch-millis and ISO strings, and how does each interact with the
  chosen policy?
- Safest default given both interop (which favors UTC) and the volume of confused users (which favors
  local)? Leaning: keep UTC as the interoperable default but make local or explicit-zone a trivially
