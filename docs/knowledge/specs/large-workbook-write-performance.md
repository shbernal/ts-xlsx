...
roughly twenty columns of plain string and number cells, then serializes it to `.xlsx`. Writing takes
...
> Spec note, not a corpus case: this is a performance and resource requirement, not a discrete value
> assertion. Wall-clock speed is machine-dependent and would make a corpus case flaky, and a naive
> repro can effectively hang, so neither belongs in the behavior corpus. The requirement is recorded
> here; a bounded-time and bounded-memory regression check belongs in a dedicated perf harness with a
...
- **Linear time, bounded memory.** Serializing a workbook to `.xlsx` completes in time roughly linear
  ...
  or hang, and memory must not scale with total cell count held live.
- **A first-class streaming write path.** For sizes where the eager in-memory model is impractical,
  meaning hundreds of thousands of rows, the library must offer streaming writes that emit rows
  ...
  shared-strings toggle, and that is the intended fast path.
...
O(n) or churn a map, per-row and per-cell object allocation, repeated string concatenation of the sheet
XML instead of streamed chunks, and unnecessary style resolution for plain cells. Competing libraries
reach tens-of-seconds writes at this scale using dense or streamed row emission and cheap
number-to-string paths. Deflate compression level can dominate cost at scale, so the default may want
...
- What time and peak-memory envelopes do we commit to, say for 500k rows by 20 columns of scalars,
  and should CI carry a perf-regression guard on time and memory rather than a value assertion?
