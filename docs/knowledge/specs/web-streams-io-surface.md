...
A developer on a modern non-Node runtime, a service worker or an edge or serverless host such as
Deno, Cloudflare Workers, or Bun, wants to read a workbook from, and write one to, a WHATWG
`ReadableStream`, `WritableStream` or `TransformStream`. These environments expose the standard
Streams API natively but may not provide Node's `node:stream` module, so an I/O surface built on Node
`Readable` and `Writable` forces users to hand-roll shims that implement just enough of the Node
stream contract to fool the library. The browser **write** side of this is captured separately
...
- The streaming **reader** accepts a WHATWG `ReadableStream` of package bytes directly as its
  ...
  stream can be parsed incrementally on Deno, Workers, Bun and the browser.
- The streaming **writer** can target a WHATWG `WritableStream`, or hand back a `ReadableStream` or
  async-iterable of package bytes the caller pipes anywhere.
- The public API is expressed against **platform-neutral byte sources and sinks** (Web Streams,
  async-iterables of `Uint8Array`), confining Node `Readable` and `Writable` to clearly Node-only entry
...
- Is the neutral contract "async-iterable of `Uint8Array`", adapting Web Streams and Node streams to
  it at the edges, or first-class Web Streams with a thin Node adapter, or both?
- Backpressure: how is the consumer's or producer's pull rate honored across the Web Streams boundary
...
- Cancellation: a consumer cancelling a `ReadableStream` read must tear the parser down promptly
  without leaking resources, which is shared with `row-iteration-early-termination`.
