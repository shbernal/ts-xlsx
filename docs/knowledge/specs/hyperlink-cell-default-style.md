...
A user writes cells whose value is a hyperlink (text plus target URL, optionally a tooltip) by
...
when visited. Users manually apply an underline plus a blue font to every link cell, which
...
- **Default against opt-in.** A default that mimics native applications is friendlier but must not
  silently clobber a font the user already set. Reasonable rule: apply the default hyperlink
  font only when the cell has no explicit font override, and let an explicit font win.
- **Named cell style against raw font.** Native applications use a named `Hyperlink` and
  `Followed Hyperlink` cellStyle in `styles.xml`, and that is what unlocks the automatic
...
- Manual per-cell font `{ underline: true, color: { argb: 'FF0000FF' } }` approximates the
  unvisited look, with no visited state.
