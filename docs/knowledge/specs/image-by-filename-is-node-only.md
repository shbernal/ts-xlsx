...
- The type and API surface should make the environment constraint discoverable, so the filesystem-path source is documented as Node-only and browser builds do not silently ship a broken code path.
...
- Should there be runtime detection, throwing a clear error, in addition to build-time separation, or is build-time separation sufficient?
