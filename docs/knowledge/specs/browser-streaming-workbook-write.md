...
- Emitted bytes are pushable to a **WHATWG `WritableStream`** so the browser flushes to disk
  progressively; equivalently, the writer hands back an async-iterable or `ReadableStream` of package
...
was rejected even though it satisfied the needed contract. The lesson: **sink and stream detection must
...
- Canonical sink shape for the public API: a WHATWG `WritableStream`, an async-iterable or
  `ReadableStream` we return, or an abstract byte-sink interface adapted per platform.
