...
is lossy and fragile, because it flattens rows to bare values, so per-cell and per-row styles,
heights, notes, merged regions, and formulas do not travel with their row, and formulas in particular
move without their references being reconsidered.
...
- Sort by one or more key columns, each with its own ascending or descending direction, over an
...
- The comparator defines a **total, predictable order across mixed value types** (numbers, strings,
  booleans, dates, rich text, hyperlink and formula result values, and empty or null cells) with
  defined placement for empties and defined case and locale handling for strings.
...
and case-sensitivity toggles. OOXML also persists a `sortState` on tables and autofilters describing
the last-applied sort, so a complete solution could optionally emit that and record the applied order
in the file, distinct from a purely in-memory reorder.

## Open questions

- Is sorting an eager in-memory row reorder, a persisted `sortState` on a table or autofilter, or both?
- Default placement of empty and null cells, first or last, and is it configurable?
...
- Formula-reference rewriting, or a value-only guarantee?
