...
error-prone, since the author must track which sheet-column index each table column maps to, and
splits one logical declaration across two APIs.

> Spec note, not a corpus case: the two-step path already works and produces correct output, so there
> is no incorrect behavior to assert. This is an ergonomics and API-shape proposal, and the durable
> value is the equivalence contract and where width belongs in the table authoring surface.
...
- **A table column definition accepts an optional `width`.** Declaring it inline is exactly
  ...
  created: same rendered column widths, same OOXML output, with a `<col>` width on the sheet and the
  table part unaffected, since width is not a table-column property in the format. The author may
  choose the ergonomic inline form with no behavioral surprise.
- **Width lives on the sheet column, not the table part.** OOXML tables (`<tableColumn>`) carry no
  ...
  the writer resolves onto the sheet column the table spans, and it does not invent a non-standard
  table attribute.
- **Composition is predictable.** An inline table-column width and an explicit worksheet-column width
  for the same column must not both silently apply and fight, so define precedence, most naturally
  that an explicit later worksheet-column width wins or that the two are the same knob, and document
  it.
