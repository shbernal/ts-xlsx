...
"page field" or report filter that renders above the pivot body as a dropdown. For example a `latest`
field with values 0 and 1 could default to showing only `latest = 1` rows, with the option to select
...
- **A pivot accepts one or more page fields** in addition to rows, columns and values: fields that
  filter the entire pivot rather than grouping it. Each named page field maps to a source field.
- **A default selected item is expressible per page field**, say defaulting the `latest` filter to `1`,
...
- **Page fields serialize into the pivot definition** as a `<pageFields>` block whose entries
  ...
  the pivot.
- **The existing row, column and value behavior is unchanged** when no page fields are supplied.
...
- Public API shape: a `pages: ['latest']` array plus an optional `pageDefaults: { latest: 1 }` map,
  against a structured per-page object. And how an unknown default value, not present in the source
  column, is handled: reject at author time, or fall back to "(All)"?
- How the pivot cache's shared items are populated so a default `item` index resolves to the intended
  value, and whether multi-select page filters, with more than one selected item, are in scope.
- Interaction with the metric or aggregation choice (`pivot-table-aggregation-metrics`) and multiple
