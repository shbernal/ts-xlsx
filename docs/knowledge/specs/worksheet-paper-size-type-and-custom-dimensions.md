...
Print and page setup exposes a paper size. Two distinct gaps show up around it.
...
Second, some real print layouts need a **custom paper size**, an explicit width and height rather than
one of the enumerated standard codes: large-format plots, receipt and label rolls, non-ISO regional
stock. Today only the enumerated codes are expressible, so those layouts cannot be authored at all.

> Spec note, not a corpus case: the type-coverage half is a type-surface requirement, enforced by a
> type-level test rather than a runtime assertion, and the custom-dimensions half is a new capability
> with no current behavior to baseline. Existing page-setup round-trip behavior (fit-to-page,
> orientation, margins) is already locked by corpus cases; this note is the umbrella policy for the
> paper-size axis.
...
- **Complete, honest `paperSize` typing.** The published type must admit every OOXML paper size code
  ...
  form is offered, such as `'A3'`, it maps to the numeric code, and the raw numeric code stays accepted
  for forward compatibility with codes we have not named.

- **Custom paper dimensions.** A caller can specify an explicit page width and height, with a unit,
  since the OOXML `pageSetup` `paperWidth` and `paperHeight` attributes take a length with a unit
  suffix such as millimetres or inches, instead of or overriding an enumerated `paperSize`. When custom
  dimensions are set, they must serialize to `paperWidth` and `paperHeight` and round-trip faithfully,
  and reading a file that already carries custom dimensions must preserve them rather than snapping to
  the nearest enumerated size. Define precedence when both a `paperSize` code and explicit dimensions
  are present; in OOXML the explicit width and height override the code.
...
- Do we ship the full named-code enum, dozens of entries most of which are rarely used, or only name
  the common ones and leave the rest as documented numeric codes?
- Reading an unknown or rare numeric code: surface it verbatim, which is forward-compatible, rather
