...
Today it silently picks the first worksheet and drops the rest, with no error, no warning, and no way
to influence the choice. Users are surprised to find only one sheet's data in the output and cannot
...
> Spec note, not a corpus case: the current output is well-formed, containing the wrong sheet
...
- CSV write accepts an option naming the worksheet to serialize, by name or index. When provided,
  that sheet is written; if the named or indexed sheet does not exist, **fail with a clear error naming
...
- When the option is omitted and the workbook has exactly one worksheet, write it with no ceremony,
  which is the current happy path.
- When the option is omitted and the workbook has **more than one** worksheet, surface the ambiguity
  rather than silently taking the first. Erroring is safer, since it admits no silent data loss, and
  fits the fork's correctness-first stance, though it is a harder break for callers relying on
  first-sheet behavior. The alternative is default-to-first with a diagnostic.

## Open questions

- Omitted selection with multiple sheets: error, or warn-and-default? Correctness against convenience.
...
- How is "active sheet" resolved against "first sheet" when both concepts exist?
- The selection option lives under the existing CSV write `options` object.
