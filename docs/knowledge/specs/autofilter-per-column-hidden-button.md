...
> Spec note, not a corpus case: this is an additive capability that does not exist yet, so there is
> no current behavior to assert a baseline against. The durable value is the desired contract and its
> design questions; it becomes a corpus case (write → inspect per-column button state → round-trip)
> once the authoring API lands, and the adapter's `autoFilter` inspection fact is extended to read
...
- **The default is unchanged.** An AutoFilter with no hidden-button columns behaves exactly as today,
  with a dropdown on every column in the range.
...
- Authoring shape: an `autoFilter: { from, to, columns: [{ column, filterButton: false }] }` object,
  or a separate per-column call? Column identification by address or by zero-based offset in the API
...
- Whether this pulls in the broader `filterColumn` surface (custom, dynamic and top-10 filters), or
  ships narrowly as just button visibility first.
