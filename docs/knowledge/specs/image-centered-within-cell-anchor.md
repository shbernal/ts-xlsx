...
> expressible over the existing OOXML anchor primitives, so it is a higher-level helper rather than a
> schema change; the durable value is the placement contract and the enabling geometry.

## Desired behavior

- An image-placement mode that **centers** an image within a target cell or range, preserving the
  image's aspect ratio and leaving symmetric margin around it: "put this image in B2:D6, centered,
  without distorting it."
- The library computes the image's intrinsic size, or a caller-supplied size or scale, measures the
  target region's pixel dimensions from the involved **column widths and row heights**, and derives a
  `oneCellAnchor` (from-cell plus inward `colOff`/`rowOff` offsets plus `ext`) so the rendered image is
  centered with equal horizontal and vertical padding.
- **Aspect-ratio-preserving fit-within (letterbox)** is the natural default; stretch-to-fill remains
  available, which is today's two-cell behavior.
- Built on existing anchor vocabulary and the column-width and row-height to pixel to EMU conversion
  already needed for pixel-accurate layout, with no new schema.

## Open questions

- API shape: a placement enum on the anchor (`top-left | fill | center-fit`), or a distinct helper
  that computes offsets from a range plus a desired size? Reuse the existing image-anchor vocabulary.
- Sizing source: intrinsic dimensions, caller width and height, or scale factor; and behavior when the
  image is larger than the region, shrink-to-fit or overflow.
- Interaction with move and size-with-cells (`editAs`): centering is computed **at write time** from
  the then-current column and row sizes and is not dynamic. Document that it does not re-center if the
  user later resizes rows or columns.
