...
open an application-produced workbook and enumerate or inspect its charts, and to declare new charts
...
- **Read path**: parse the chart parts (DrawingML chart XML under the drawing relationships) and
  ...
  with their value, category and name range references, axis configuration, and the drawing anchor
  (cell-anchored position and size) tying the chart to the sheet.
...
- **Round-trip**: a chart present in a loaded workbook survives a read-then-write unchanged, both in
  the passthrough case and, once modeled, the typed case, with series range references intact.

## Open questions

- How much of the chart type space is first-class rather than passthrough-only initially? Bar, line,
  pie and scatter cover most demand.
- Series data: only cell-range references (live data), or also inline or cached literal values?
- Is the chart model immutable-by-default with explicit mutation entry points, matching the fork's
  stance, and how does it compose with the drawing and anchor model shared with images?
- Combo charts, secondary axes, and stock, radar and surface types: in scope or deferred?
