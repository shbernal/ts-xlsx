...
A caller wants to duplicate not a single row but a contiguous run of rows as a group, say copying a
5-row template block and pasting it 10 times to build a repeating layout, in one operation rather
than looping a single-row duplicate. Today the duplication affordance handles one row, with a repeat
...
> assertable by reading back the pasted range's values, formulas, styles and heights.
...
- **Single-row duplication remains a special case** (length 1, one copy) and behaves exactly as the
  existing single-row duplicate does, so the block form generalizes it without breaking it.
- **Each copy is faithful.** Cell values, formulas, number formats, fonts, fills, borders, row heights,
  ...
  the block are re-created per copy, and the merge and ref bookkeeping stays consistent, the same
  discipline the row-insert and duplicate cases already exercise for merges and table and image refs.
...
- **Interaction with structured content is defined.** Duplicating a block that overlaps or extends a
  table, or that shifts anchored images or defined names below it, must keep the package valid and the
  refs consistent, or reject with a clear error if the operation would corrupt a table.

## Open questions

- Insert-and-shift or overwrite: does pasting copies push existing rows down (insert semantics) or
