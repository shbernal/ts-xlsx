...
is deep in the tree, consumers cannot upgrade it directly and are forced into fragile package-manager
override and resolution tricks to make `audit` pass. They want the library itself to carry a
dependency tree so small and current that this never happens.
...
- The dependency tree is **small and modern by default**, and every transitive package earns its
  place. A dependency audit is part of CI and is expected to stay green, since a founding reason for
  the fork was upstream's rotting transitive dependencies.
- **Prefer platform built-ins over third-party packages** for commodity primitives. A UUID generator
  is the canonical example: use the runtime's built-in `crypto.randomUUID()` (Node 14.17 and later,
  and the Web Crypto equivalent in browsers) instead of pulling a `uuid`-style dependency. Every
  eliminated dependency is one fewer advisory consumers can inherit.
- **No deep, un-upgradable pins.** The library must never wedge consumers into override or resolution
...
- Supply-chain hygiene is a **maintained invariant**, not a one-time cleanup, so new dependencies are
...
- Where a browser target lacks a built-in a Node target has: conditional exports, a tiny inlined
  shim, or requiring the host to provide it?
- CI policy: fail the build on a `moderate` or higher advisory, or gate on `high` and above with a
  tracked allowlist?
