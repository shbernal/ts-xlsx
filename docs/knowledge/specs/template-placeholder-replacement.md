...
expanding rows or blocks driven by array data).
...
> prototyped it composes existing primitives (cell read and mutate, `spliceRows` for `foreach`
> expansion, image insertion) rather than needing new low-level format support, so it is a layer above
> the document model, worth naming as a distinct, optional surface.

## Desired behavior

- **Text substitution.** A delimited token, mustache-style `{{name}}`, found in a cell's text or value
  is replaced by the bound value. Tokens resolve in every sheet and every cell they appear in. A token
  embedded within a larger string replaces in place, keeping the surrounding text, and the cell's style
  and number format are preserved across the substitution.
- **Image substitution.** An image placeholder, a token or a marked cell or anchor, is replaced by an
...
- **Repetition (`foreach`).** A block or row region bound to a collection is expanded once per item,
  shifting subsequent rows down in the `spliceRows`-style insert, with per-item tokens resolving
  against each element. Merges, styles, and row heights in the template block are carried to each
  expansion.
- **Conditional inclusion (`if`/`else`).** A region guarded by a condition is kept or dropped, with its
...
- **Missing-binding policy is explicit.** A token with no bound value resolves to a defined outcome
  (empty string, left-as-is, or error), chosen and documented rather than silently divergent.

## Open questions

- Is templating in the library's core, or a separate opt-in package built on the document API? A
  separate layer keeps the core a pure reader and writer and matches the "small, composable" stance.
- Token syntax and escaping: mustache `{{ }}` by default, but how does a caller emit a literal `{{`?
  How are nested paths (`{{customer.name}}`) and formatting directives handled?
- Expansion semantics: how do `foreach` and `if` interact with tables, merged regions, charts, which
  reference cell ranges, and defined names when rows are inserted or removed?
- Type fidelity: a substituted value that is a number or date must become a typed cell, not a string,
  so downstream formulas and formats work.
