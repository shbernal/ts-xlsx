...
- `get themeColors(): ThemeColorScheme;` — The colour scheme every `theme="n"` reference in this workbook resolves against — the preserved theme's `<a:clrScheme>`, or the Office default when the workbook carries no theme of its own. Note the slot *order*: `theme="0"` is `lt1` and `theme="1"` is `dk1`, which is not the order the slots appear in the theme part. See `THEME_COLOR_SLOTS`.
