...
- **silently runs stale p-code** and ignores the new source entirely (mismatched p-code). That is the
  worst case, because the file "opens clean".
...
- **Trust access to the VBA project object model** must be ON:
  ...
  model"). Off by default, and VBIDE automation silently fails without it.
...
```jsonc
{
  ...
  // right mode for editing a real workbook's code-behind (ThisWorkbook/Sheet1), where the modules bind
