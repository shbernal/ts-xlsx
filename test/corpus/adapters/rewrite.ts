...
const {CompoundFile} = await loadModule<typeof import('../../../src/vba/cfb.ts')>('vba/cfb');
...
const vbaU16 = (n: number) => [n & 0xff, (n >> 8) & 0xff];
const vbaU32 = (n: number) => [n & 0xff, (n >> 8) & 0xff, (n >> 16) & 0xff, (n >> 24) & 0xff];
const vbaAscii = (s: string) => [...s].map((c) => c.charCodeAt(0));
const vbaUtf16 = (s: string) => [...s].flatMap((c) => vbaU16(c.charCodeAt(0)));
const vbaRec = (id: number, data: number[]) => [...vbaU16(id), ...vbaU32(data.length), ...data];

interface VbaFixtureModule {
...
const VBA_FIXTURE_MODULES: VbaFixtureModule[] = [
  {
    name: 'ThisWorkbook',
    document: true,
...
const VBA_FIXTURE_CODE_PAGE = 1251;
...
function buildVbaFixtureBin(): Uint8Array {
  const dir: number[] = [];
...
function buildVbaFixturePackage(bin: Uint8Array): Uint8Array {
...
function vbaIndexOfBytes(haystack: Uint8Array, needle: Uint8Array): number {
