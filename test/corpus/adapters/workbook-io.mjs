...
export async function inspectPackage(spec) {
  ...
  // Read every non-directory part into a { path: xmlString } map, then delegate to the
  // shared, implementation-blind extractor so this adapter and the rewrite adapter derive
  // identical facts from their respective writers' output.
  const partMap = {};
  for (const name of Object.keys(zip.files)) {
    if (!zip.files[name].dir) partMap[name] = await zip.file(name).async('string');
  }
  return packageFacts(spec, partMap);
}
