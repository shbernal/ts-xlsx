...
| `roundtripInspectPackage(spec)` | Like `inspectPackage`, but returns the facts of the package after a full write→read→write cycle (the spec is written, reloaded into a fresh model, and re-emitted) — for asserting writer-materialized content (e.g. a table's totals row) survives a round-trip unchanged, neither dropped on read nor duplicated/clobbered on re-write. |
...
| `carrySheetImagesAcrossWorkbooks()` | Carry an image-bearing sheet into a DIFFERENT workbook — `model` for its content, `exportImages`/`importImages` for its pictures — write the destination and report `{dstMediaCount, allEmbedsResolve, reReadImageCount, reReadAnchor, reReadHasBackground, srcStillShowsImage, danglingAnchorError}` — for asserting a carried picture is registered in the destination and its embed resolves to a real media part (a dangling embed is a silently broken image, not an error), and that an anchor still holding a foreign workbook's media id is refused by name at write time. |
...
| `xmlCharacterSafetyReport(where, text)` | Author a string XML 1.0 cannot carry (C0 control, noncharacter, lone surrogate) into `'cell-text'`/`'formula-result'`/`'sheet-name'`; return `{writeOk, writeError, partsWithRawChar, emittedText, readValue}` — for asserting a cell value is `_xHHHH_`-escaped and reads back intact, while a structural string is refused rather than written into a damaged package. |
| `escapeDecodeReport(rel, refs)` | Read a fixture holding `_xHHHH_` escapes eagerly, through the streaming reader, and after a write/read round-trip; return `{eager, streaming, roundtrip}` as A1-reference maps of decoded text — for asserting the decode is one left-to-right pass, agrees between the two readers, and inverts the escape. |
| `escapedTextCarrierRoundtrip(text)` | Author `text` into a legacy note's body and two rich-text runs, round-trip; return `{note, runs}` — for asserting the other `<t>` carriers escape and decode like a plain string cell. |
| `threadedCommentEscapeReport(rel, refs)` | Read a fixture whose threaded-comment bodies hold `_xHHHH_` escapes and report each anchor's message text as read and after a write→read through our own writer; return `{eager, roundtrip}` — for asserting the 2018 `<text>` decodes like a `<t>` and that the escape and the decode are inverses. |
| `authoredThreadedCommentEscape(text)` | Author `text` into a threaded comment, write, and read back; return `{emittedText, rawInPart, readText}` — for asserting a character XML cannot carry reaches the part as its escape (in the thread part and in the legacy fallback alike), never raw. |
...
| `frozenHeaderViewStateReport()` | Freeze the top row above grouped, hidden columns on the first of two sheets, write, and report the package's view state → `{bookViewCount, bookViewsBeforeSheets, windowWidth, windowHeight, selectedSheets, outlineLevelCol, outlineLevelRow, paneEmitted}` — for locking that the written package carries the window rect, the single `tabSelected` sheet, and the outline depth a consumer initialises its pane and outline-bar layout from. |
...
| `headerFooterEscapeReport(rel)` | Read a fixture whose `<headerFooter>` children hold `_xHHHH_` escapes and report the six header/footer slots as read and after a write→read through our own writer; return `{eager, roundtrip}` — for asserting a print header decodes like a `<t>` and that the escape and the decode are inverses. |
| `authoredHeaderFooterEscape(text)` | Author `text` as a sheet's odd header, write, and read back; return `{emitted, rawInPart, read}` — for asserting a character XML cannot carry reaches the part as its escape, never raw, and that the `&`-prefixed section codes survive it. |
...
| `loadMutateCellFacet(facet)` | Load a workbook whose two cells shared one style record, set one style facet (`alignment`\|`numFmt`\|`protection`) on one cell via its setter → `{facet, target, sibling, original, bled, diskSibling, diskBled}` — for asserting copy-on-write: mutating one cell's style never edits the aliased shared record a sibling still points at. Extends the fill/font/border isolation family. |
| `streamReadManySheets(count?)` | Write a `count`-worksheet workbook and stream-read it from disk → `{written, emitted, error, first, last}` — for asserting the streaming reader emits every worksheet regardless of sheet count rather than losing the tail (or throwing) once a worksheet part is parsed before the workbook model at scale. |
| `streamVsEagerColumnHidden()` | Write a workbook with a hidden column, read it eagerly and via the streaming reader → `{eager, stream, error}` each `{col1, col2, col3}` hidden flags — for asserting the streaming reader surfaces a column's hidden flag, agreeing with the eager oracle. Column companion to `streamVsEagerRowHidden`. |
| `quotePrefixReport()` | Request the quote-prefix cell-format flag via the style path, write, round-trip → `{writtenQuotePrefix, anyQuotePrefix, reloaded}` — for asserting the produced xf carries `quotePrefix` and a read/modify/write round-trip preserves it (the flag forces a cell's content to literal text). |
| `namedStyleFillReport(rel, cellAddr?, sheetName?)` | Load a fixture whose cell fill is carried through a NAMED cell style (a `cellXfs` `xfId` into `cellStyleXfs`, fill defined only in the named-style layer), read the cell's resolved fill, then round-trip → `{readFill, srcCellStyleXfsCount, roundtripCellStyleXfsCount, roundtripCellHasXfIdLink}` — for asserting a reader resolves the fill through the named style and a writer preserves the `cellStyleXfs` layer and the `xfId` link. |
| `listValidationSourceRangeAcrossRows(count?, source?)` | Apply one cross-sheet list validation (source range on another sheet) to `count` stacked cells, write, reload → `{source, count, formulae, allIdentical, sqrefBlocks}` — for asserting per-cell application does not relative-shift the source range (every cell keeps the exact reference; identical rules may collapse to one `sqref`). |
| `worksheetPasswordProtectionReport(password?)` | Password-protect a worksheet in the current runtime, serialize twice → `{threw, algorithm, hasHash, hasSalt, spinCount, selectLockedCells, selectUnlockedCells, saltsDiffer}` — for asserting password hashing runs on real platform randomness (no browser-only secure-random shim throwing under Node) and emits a valid, salted protection element. |
| `removeImageReport(range?)` | Add two images, attempt to remove one by media id → `{supported, before, after, removedGone, othersSurvive}` — for asserting a worksheet exposes a way to remove a previously-added image, dropping the target and preserving the rest. |
