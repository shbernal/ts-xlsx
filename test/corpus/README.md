...
| `readFixtureDefinedNames(rel)` | Read a fixture and report the workbook-level defined names the reader exposes → `{names: {<name>:[ranges]}, count}` — for asserting a full-row/full-column-span named range a real file declares is read back, not silently dropped by over-strict range-address validation. (`roundtripWorkbook`'s model also carries a `definedNames` map for the write→read path.) |
