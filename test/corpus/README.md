...
| `mutateWorksheet({cells, ops, read})` | Build a fresh sheet, apply structural mutations (`spliceRows`/`spliceColumns`/`mergeCells`), and return `{rowCount, columnCount, cells, merges, error}` — for asserting in-memory model edits behave predictably (a throwing op, e.g. an overlapping merge, is reported as `error`, not propagated; `merges` lets a case assert a splice SHIFTS a merged range and keeps it merged, rather than stranding it at its original indices). |
